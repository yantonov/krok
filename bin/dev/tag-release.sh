...
# Only the package version is expected to match: dependency versions are written
# as 'name = { version = "x.y.z" }' and rust-version does not start the line with
...
# refuses to tag something that does not even compile. Both files go back if it
# does not, rather than leaving the version raised for a release that was never
# made. Cargo.lock is taken from the commit, which is what it held: a tree with
# anything of its own in it was refused above.
echo "==> Checking..."
if ! cargo check --quiet; then
  mv Cargo.toml.bak Cargo.toml
  git checkout -- Cargo.lock
  echo "cargo check failed, left at ${CURRENT}"
  exit 1
fi
